...
function preliminaryFilenameCheck() {
    ...
    // banned characters [" ", "__"]
    var doIt = true;
    if (myText.includes(" ")) {
        doIt = false;
        createCheck(true, "space found", false)
    }
    else {
        createCheck(false, "no spaces found", false)
    }

    if (myText.includes("__")) {
        doIt = false;
        createCheck(true, "double underscore found", false)
    }
    else {
        createCheck(false, "no double underscores", false)
    }
    ...
    // last 3 characters
    /*
    if(objectClicked.key == 'E_FSTO'){
        // e fsto can have up to 3 numerical characters on end
        const lastUnderscore = myText.lastIndexOf('_')
        const lastPosition = myText.length
        const finalCharacters = myText.slice(lastUnderscore, lastPosition)
        const characterAfterUnderscore = finalCharacters[1];
        const expectedCharacterAfterUnderscore = objectClicked.lastThreeCharacters[1];
        const finalNumbers = finalCharacters.slice(2, finalCharacters.length)
        if((characterAfterUnderscore == expectedCharacterAfterUnderscore) && (parseInt(finalNumbers) > -1) ){
            createCheck(false, " ends with correct characters, " + finalCharacters, false)
        }
        else{
            doIt = false;
            createCheck(true, " does not end with format like " + objectClicked.lastThreeCharacters, false)
        }
    }
    */
    //else{
    if (myText.slice(-3) != objectClicked.lastThreeCharacters) {
        doIt = false;
        createCheck(true, " does not end with " + objectClicked.lastThreeCharacters, false)
    }
    else {
        createCheck(false, " ends with " + objectClicked.lastThreeCharacters, false)
    }
    //}
